...
if __name__ == '__main__':
    ...
    logger.info('='*200)
    logger.info(f'Ablating SM performance on {dataset} with {model} and {num_observed} observed configurations... Executing {num_seeds} runs.')
    logger.info('='*200)
    ...
    for seed in range(num_seeds):
        ...
        LLM_SM = LLM_DIS_SM(task_context, n_gens=10, lower_is_better=lower_is_better, 
                                ...
                                verbose=False, rate_limiter=rate_limiter)
        y_mean, y_std, cost, time_taken = asyncio.run(LLM_SM._evaluate_candidate_points(observed_configs, observed_fvals, candidate_configs, use_context=False, use_feature_semantics=False))
        ...
        logger.info(f"[LLAMBO_Uninf] RMSE: {rmse:.4f}, R2 score: {r2:.4f}, NLL: {nll:.4f}, "
